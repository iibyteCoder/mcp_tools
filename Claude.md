...
1. **边界清晰** - `mysql-client` 负责协议、解析、策略和执行；`mysql-command` 负责 CLI 输入输出与 profile 管理
...
# 静态检查
uv run ruff check libs/mysql-client packages/mysql-command
uv run mypy libs/mysql-client/src packages/mysql-command/src
```
