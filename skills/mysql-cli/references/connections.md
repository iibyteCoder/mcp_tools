...
Profile operations manage local connection metadata and explicit validation. The implemented `server` and `schema` commands consume a selected profile and perform read-only metadata inspection. The `sql` routes parse and classify one statement, then execute it through one session against the explicitly selected profile. `sql read`, `sql explain`, and read-only `sql benchmark`/`sql compare` never issue writes; `sql write` performs only the supplied authorized statement and applies its explicit transaction action. Profile commands do not execute application SQL, and inspection commands do not silently edit profiles, bindings, or session databases.

All successful JSON calls return one structured document. A nonzero exit status or `ok: false` is a failure. Keep stdout free of logs. Stable error codes include `invalid_argument`, `profile_not_found`, `authentication_failed`, `connection_failed`, `timeout`, `cancelled`, `database_not_found`, `execution_failed`, `comparison_failed`, and `unsupported_sql`.
